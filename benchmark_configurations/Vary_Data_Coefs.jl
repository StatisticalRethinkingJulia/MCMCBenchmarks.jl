...
"""
 function benchmark(samplers,simulate,Nd,Nc,Nreps=100;Nsamples=2000,Nadapt=1000,delta=.8)
     results = DataFrame()
     for nd in Nd
         for nc in Nc
             benchmark!(samplers,results,simulate,Nreps;Nd=nd,Nc=nc,Nsamples=Nsamples,Nadapt=Nadapt,delta=delta)
         end
     end
     return results
 end
